<div x-data="{ open: false }" class="px-6 py-1 ">
                <a x-on:click="open = ! open"
                    ...
                    <span class="mr-2 text-lg text-indigo-500 uppercase dark:text-white">
                        Documents soumis
                    </span>
                </a>
                ...
                <div>
                    <div x-show="open" x-transition>
                        ...
                        <div class="grid w-full grid-cols-1 gap-6 p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            ...
                            @if ($record->autres)
                                @forelse($record->references as $key=>$dossier)
                                    @include('components.references')
                                @empty
                                @endforelse
                            @endif
                            @if ($record->confidential)
                                @forelse($record->confidential as $key=>$dossier)
                            ...
                            @endif
                            @if ($record->autres)
                                @forelse($record->autres as $key=>$dossier)
